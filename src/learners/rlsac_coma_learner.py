...
class SACCOMALearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # TODO: reshape from 1 to 8
        q_taken = th.gather(q_vals, dim=1, index=actions.reshape(-1, 8)).prod(1)
        pi_taken = th.gather(pi, dim=1, index=actions.reshape(-1, 8)).prod(1)
        # pi_taken[mask == 0] = 1.0
    ...
    def _train_critic(self, batch, rewards, terminated, actions, avail_actions, mask, bs, max_t):
        ...
        target_q_vals = target_q_vals.repeat(1, 1, 3, 1)
        ...
        running_log["q_taken_mean"].append((q_taken).sum().item() / mask_elems)
        # running_log["q_taken_mean"].append((q_taken * mask).sum().item() / mask_elems)
        running_log["target_mean"].append((targets).sum().item() / mask_elems)
